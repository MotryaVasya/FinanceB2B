...
async def get_statistics_from_month(month: int, user_id: int) -> dict:
    ...
    try:
        ...
        logging.error(
            f"Ошибка статуса от API (статистика за месяц): {e.response.status_code} - {e.response.text}"
        )
        ...
        try:
            ...
            pass
        raise ValueError(
            f"API вернул ошибку {e.response.status_code}: {api_error_detail}"
        )
...
async def get_statistics_from_period(data: dict) -> dict:
    ...
    try:
        ...
        logging.error(
            f"Ошибка статуса от API (статистика за период): {e.response.status_code} - {e.response.text}"
        )
        api_error_detail = e.response.text
        try:
            api_error_detail = e.response.json().get("detail", api_error_detail)
...
async def select_day(callback: types.CallbackQuery):
    ...
    if user_id not in user_period:
        await callback.message.edit_text(
            "🤷 Пожалуйста, начните выбор периода сначала, нажав 'За период'."
        )
        await callback.answer(
            "Ошибка: состояние выбора периода не найдено.", show_alert=True
        )
        return
    ...
    try:
        ...
        # Определение максимального дня для выбранного месяца и года
        year = user_period[user_id].get(f"{step}_year", datetime.now().year)
        month = user_period[user_id].get(f"{step}_month", datetime.now().month)
        max_day = monthrange(year, month)[1]

        if day > max_day:
            await callback.message.edit_text(
                f"⚠️ Ошибка: В {get_month_name(month)} {year} не может быть {day} дней.  Пожалуйста, выберите день заново."
            )
        ...
        logging.warning(
            f"Invalid day callback data from user {user_id}: {callback.data}"
        )
        await callback.answer("Некорректные данные дня.", show_alert=True)
    except TelegramAPIError as e:
        logging.error(
            f"Telegram API error during day selection for user {user_id}: {e}"
        )
...
async def select_month(callback: types.CallbackQuery):
    ...
    if not current_step or f"{current_step}_day" not in user_period.get(
        user_id, {}
    ):
        await callback.message.edit_text(
            "🤷 Пожалуйста, начните выбор периода с выбора дня."
        )
        await callback.answer(
            "Ошибка: предыдущий шаг (выбор дня) не выполнен.", show_alert=True
        )
        return
    ...
    try:
        ...
        logging.warning(
            f"Invalid month callback data from user {user_id}: {callback.data}"
        )
        await callback.answer("Некорректные данные месяца.", show_alert=True)
    except TelegramAPIError as e:
        logging.error(
            f"Telegram API error during month selection for user {user_id}: {e}"
        )
        await callback.answer("Ошибка связи с Telegram. Попробуйте еще раз.")
...
async def select_year(callback: types.CallbackQuery):
    ...
    user_id = callback.from_user.id
    current_step = user_period.get(user_id, {}).get("step")
    if not current_step or f"{current_step}_month" not in user_period.get(
        user_id, {}
    ):
        await callback.message.edit_text(
            "🤷 Пожалуйста, начните выбор периода с выбора дня и месяца."
        )
        await callback.answer(
            "Ошибка: предыдущий шаг (выбор месяца) не выполнен.", show_alert=True
        )
        return
    ...
    try:
        ...
        # Проверяем корректность дня с учетом выбранного года и месяца
        day = user_period[user_id].get(f"{step}_day")
        month = user_period[user_id].get(f"{step}_month")
        max_day = monthrange(year, month)[1]
        if day > max_day:
            await callback.message.edit_text(
                f"⚠️ Ошибка: В {get_month_name(month)} {year} не может быть {day} дней. Пожалуйста, выберите день заново."
        ...
        if step == "from":
            ...
            from_month_name = get_month_name(
                user_period[user_id]["from_month"], case="genitive"
            )
            await callback.message.edit_text(
            ...
            try:
                ...
                to_dt = to_dt + timedelta(days=1)  # Include the end date
                ...
                logging.info(
                    f"Requesting stats for user {user_id} for period {from_date_iso} to {to_date_iso}"
                )

                stats = await get_statistics_from_period(data)

                # --- Динамическое формирование текста ---
                income_data = stats.get("income", {})
                expense_data = stats.get("expense", {})
                income = (
                    income_data.get("sum", 0)
                    if isinstance(income_data, dict)
                    else income_data
                )
                expense = (
                    expense_data.get("sum", 0)
                ...
                if top_categories:
                    ...
                    for idx, item in enumerate(top_categories):
                        ...
                        text += f"{idx + 1}) {category_name} — {full_sum} ₽\n"
                        if idx >= 2:
                            break
                else:
                    text += "Нет данных\n"
                ...
                await callback.message.answer(
                    text, reply_markup=get_menu_only_reply_keyboard()
                )
                await callback.answer("Статистика сформирована.")  # Отвечаем на колбэк
                logging.info(
                    f"Successfully displayed period stats for user {user_id} and cleared state."
                )
                del user_period[
                    user_id
                ]  # Очищаем состояние после успешного получения статистики

            except (KeyError, ValueError) as e:
                logging.error(
                    f"Data or API response error for period stats user {user_id}: {e}. Current state: {user_period.get(user_id)}"
                )
                error_message = (
                    "🤷 Произошла ошибка при обработке данных: {e}. Пожалуйста, попробуйте выбрать период заново."
                )
                ...
                if user_id in user_period:
                    del user_period[user_id]  # Очищаем состояние
                await callback.answer("Ошибка данных.", show_alert=True)
            except ConnectionError as e:
                logging.error(
                    f"API connection error getting period stats for user {user_id}: {e}"
                )
                await callback.message.edit_text(
                    f"Не удалось подключиться к серверу статистики: {e}. Попробуйте позже.",
                    reply_markup=None,
                )  # Удаляем инлайн-кнопки
                if user_id in user_period:
                    del user_period[user_id]  # Очищаем состояние
                ...
                logging.exception(
                    f"Unexpected error during period stats processing for user {user_id}: {e}"
                )
                await callback.message.edit_text(
                    "Произошла непредвиденная ошибка при формировании статистики. 😥",
                    reply_markup=None,
                )  # Удаляем инлайн-кнопки
                if user_id in user_period:
                    del user_period[user_id]  # Очищаем состояние
        ...
        logging.warning(
            f"Invalid year callback data from user {user_id}: {callback.data}"
        )
        await callback.answer("Некорректные данные года.", show_alert=True)
    except TelegramAPIError as e:
        logging.error(
            f"Telegram API error during year selection/processing for user {user_id}: {e}"
        )
        await callback.answer("Ошибка связи с Telegram. Попробуйте еще раз.")
    except Exception as e:
        ...
        try:
            await callback.message.edit_text(
                "Произошла серьезная ошибка при обработке вашего запроса. 😥",
                reply_markup=None,
            )  # Удаляем инлайн-кнопки
            if user_id in user_period:
                del user_period[
...
async def show_statistic_for_month(callback: types.CallbackQuery):
    ...
    try:
        ...
        logging.warning(
            f"Invalid data or API response error for user {user_id}: {callback.data} - {e}"
        )
        error_message = f"Некорректные данные для месяца ({e})."
        if isinstance(e, ValueError) and "API вернул ошибку" in str(e):
            error_message = f"😥 Не удалосьполучить статистику: {e}"
        await callback.answer(error_message, show_alert=True)
        try:
            await callback.message.edit_text(
                f"Ошибка: {error_message}", reply_markup=None
            )  # Удаляем инлайн-кнопки
        except TelegramAPIError:
            pass
    except ConnectionError as e:
        logging.error(
            f"API connection error getting monthly stats for user {user_id}: {e}"
        )
        ...
        logging.error(
            f"Telegram API error during monthly stats display for user {user_id}: {e}"
        )
        await callback.answer("Ошибка связи с Telegram. Попробуйте еще раз.")
    except Exception as e:
        logging.exception(
            f"Unexpected error during monthly stats processing for user {user_id}: {e}"
        )
        await callback.answer("Внутренняя ошибка сервера.", show_alert=True)
        try:
            await callback.message.edit_text(
                "Произошла непредвиденная ошибка. 😥", reply_markup=None
            )  # Удаляем инлайн-кнопки
        except TelegramAPIError:
            pass
